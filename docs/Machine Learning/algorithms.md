...
Notice:
- $w^TS_W​w$ and $w^TS_Bw$ are scalars, thus we get $$\alpha S_Ww = \beta S_Bw$$If we take $\lambda = \alpha / \beta$ we get $S_B​w=λS_W​w$ which the generalized eigenvalue problem:
$$S_W^{−1}​S_B​w=λw$$
